...
bt <- bike_train[,2:9]
bt_cor <- cor(bt)
corrplot(bt_cor, method = 'color', addCoef.col="black")
...
full.fit.step.log<-lm(log(count)~humidity + windspeed + temp + atemp + weather  + workingday + holiday + season
                ,data=bike_train)

# R2 .2576
summary(full.fit.step.log)

# AIC compare - 36329.35
AIC(full.fit.step.log)

# BIC compare - 36402.3
BIC(full.fit.step.log)

# Using Lasso via SAS, the suggested model is below RMSE (Lasso): 122.6  |  RMSE LM approach: 121.3
SAS.Lasso <- lm(count ~ humidity + windspeed + atemp + weather + workingday + holiday + season ,data=bike_train)
summary(SAS.Lasso)

# build the model for scoring
...
# store the model for scoring
model  <- lm(formula = model.base.formula, data = bike_train)
                            
# apply the model to the test data set                             
bike_test$count <- predict.lm(model, bike_test)
...
# submit <- bike_test %>% subset(select=c(datetime, count))
