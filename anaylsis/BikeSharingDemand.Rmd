...
# Chance - New Input - results
coef(lasso.model)[, 100]
lasso.model.reg <- lm(count ~ season + holiday + workingday + weather + temp + humidity + windspeed + year + month + day + hour, data = train.mod.1 )

summary(lasso.model.reg)
AIC(lasso.model.reg)
BIC(lasso.model.reg)
RMSE <- function(error) { sqrt(mean(error^2)) }
RMSE(lasso.model.reg$residuals)
...
We prepared three competing models across the bike sharing training data set using the following approaches: 1) stepwise, 2) LASSO, and 3) a custom approach. The results from all three modeling exercises are very similar, and no one model is a key front runner. That said, the Stepwise model does perform slightly better using the evaluation measures: AIC, BIC, RMSE, and Adjusted R2. For detailed results, please reference the table below:

Model          |  RMSE  |  Adjusted R2  |  AIC  |  BIC  |
---------------|--------|---------------|-------|-------|
1. Custom      |  0.642 |  0.8225       | 19676 | 20135 |
2. LASSO       |  0.599 |  0.7915       | 21642 | 23784 |
3. Stepwise    |  0.599 |  0.8224       | 19668 | 19996 |
